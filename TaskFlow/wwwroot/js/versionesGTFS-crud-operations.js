...
const handleEditVersionModal = editButton => {
  ...
  const versionDescripcion = document.getElementById(`${versionId}-editVersion`).parentElement.parentElement.children[0].innerText;
  const versionFechaInicio = document.getElementById(`${versionId}-editVersion`).parentElement.parentElement.children[1].innerText;
  const versionEstado = document.getElementById(`${versionId}-editVersion`).parentElement.parentElement.children[2].innerText;
...
function setFormAttributes(form, versionId, handler) {
  ...
  form.action = `/Administration/GTFS/VersionCRUD?handler=${handler}&id=${versionId}`; // Define la acción del formulario
}
