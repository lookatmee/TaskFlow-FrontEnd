...
(function () {
  // Inicialización de DataTable para listar stops
  $('#stopTable').DataTable({
    order: [[1, 'desc']], // Ordena por la segunda columna (Código Parada) de manera descendente
    ...
    language: {
      sLengthMenu: '_MENU_',
      search: '',
      searchPlaceholder: 'Buscar Parada',
      paginate: {
    ...
    columnDefs: [
      ...
      {
        targets: 1, // Código Parada
        responsivePriority: 4
      },
      {
        targets: 2, // Nombre Parada
        responsivePriority: 3
      },
      {
        targets: 3, // Posición GPS (Google Maps link)
        responsivePriority: 9
      },
      {
        targets: 4, // Tipo Ubicación
        responsivePriority: 5
      },
      {
        targets: 5, // Embarque silla de ruedas
        searchable: false,
        orderable: false,
      ...
      details: {
        display: $.fn.dataTable.Responsive.display.modal({
          header: function (row) {
            var data = row.data();
            var stopName = data[2];
            return 'Detalles de la Parada ' + stopName;
          }
        }),
        type: 'column',
