...
(function () {
  ...
  const handleEditCompanyModal = editButton => {
    ...
    const companyRow = document.getElementById(`${companyId}-editCompany`).parentElement.parentElement;

    const companyName = companyRow.children[1].innerText;
    const companyNameLargo = companyRow.children[2].innerText;
    const companyDireccion = companyRow.children[3].innerText;
    const companyTelefono = companyRow.children[4].innerText;
    const companyEmail = companyRow.children[5].innerText;
    const companySubsistema = companyRow.children[6].innerText.trim(); // Asegurarse de eliminar espacios
    const companyAgency = companyRow.children[7].innerText.trim(); // Asegurarse de eliminar espacios
        ...
        buttons: [
          {
            ...
            exportOptions: {
              columns: [1, 2, 3, 4, 5, 6, 7], // Columnas que serán exportadas
              format: {
